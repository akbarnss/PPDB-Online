...
<?php
	...
	$dset = mysqli_query($colok, "SELECT Pendaftaran FROM setting WHERE id_setting=1");
	$dstp = mysqli_fetch_array($dset);
	$dtp = explode(" - ",$dstp['Pendaftaran']);
	//--pendaftaran awal
	$ddtpb = explode(" ",$dtp[0]); // memisahkan menjadi tanggal dan jam
	$dttpp = explode("/",$ddtpb[0]); // mecah tanggal 
	$djpbp = explode(":",$ddtpb[1]); // mecah jam
	
	//$jamb="";
	if ($ddtpb[2] == "PM" AND $djpbp[0] < 12) {
		$djamp = $djpbp[0] + 12;
	}
	elseif ($ddtpb[2] == 'AM') {
		$djamp = $djpbp[0];
	...
	$jamb = $dttpp[2]."-".$dttpp[1]."-".$dttpp[0]." ".$djamp.":".$djpbp[1].":00";
	
	//--pendaftaran akhir
	$dtpep = explode(" ",$dtp[1]); // memisahkan menjadi tanggal dan jam
	$ddtpep = explode("/",$dtpep[0]); // mecah tanggal 
	$djpep = explode(":",$dtpep[1]);
	//$jame="";
	if ($dtpep[2] == "PM" AND $djpep[0] < 12) {
		$djamsp = $djpep[0] + 12;
	}
	elseif ($dtpep[2] == 'AM') {
		$djamsp = $djpep[0];
	...
	$jams = $ddtpep[2]."-".$ddtpep[1]."-".$ddtpep[0]." ".$djamsp.":".$djpep[1].":00";
	
	if($tgl_saiki < $jamb) {
?>
		<div class="callout callout-info">
			<h4>Pendaftaran BELUM DIBUKA</h4>
			<p>Pendaftaran Peserta Didik Baru Dibuka mulai tanggal <?php echo $dtp[0]?></p>
		</div>
<?php
	}
	elseif($tgl_saiki>$jams){
?>
		<div class="callout callout-danger">
			<h4>Pendaftaran SUDAH TUTUP</h4>
			<p>Pendaftaran Peserta Didik Baru sudah diTUTUP tanggal <?php echo $dtp[1]; ?></p>
		</div>
<?php
	}
	elseif(($tgl_saiki >= $jamb) AND ($tgl_saiki<=$jams)) {
?>
